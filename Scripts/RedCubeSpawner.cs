...
public class RedCubeSpawner : CubeSpawner {
    ...
    {
        if (StageChange.getTotalStages() > 3)
        {
            base.startSpawning();
            base.setDelayTimer(delayTimer);
        }
        if (StageChange.getTotalStages() < 15)
            base.setDelayTimer(base.getDelayTimer() - (0.15f * (1 / Mathf.Sqrt(StageChange.getTotalStages()))));
        else
